                    ...
                    const ComPtr<ID3D12Device>& device)
{
    ...
    if (vError != nullptr)
    {
        auto vErrorMessage = static_cast<const char*>(vError->GetBufferPointer());
    ...
    if (pError != nullptr)
    {
        auto pErrorMessage = static_cast<const char*>(pError->GetBufferPointer());
    ...
    //root signature
    {
        ...
        if (FAILED(
            device->CheckFeatureSupport(D3D12_FEATURE_ROOT_SIGNATURE, &versionRootSignatureDesc, sizeof(
        ...
        ThrowIfFailed(D3D12SerializeVersionedRootSignature(&versionRootSignatureDesc, signature.GetAddressOf(),
                                                           error.GetAddressOf()));
